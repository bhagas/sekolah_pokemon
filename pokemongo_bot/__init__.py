...
class PokemonGoBot(object):
    ...
    def _set_starting_position(self):
        ...
        self.api.set_position(*self.position)
        logger.log('')
        logger.log(u'[x] Alamat ditemukan: {}'.format(self.config.location.decode(
            'utf-8')))
        logger.log('[x] Posisi digame : {}'.format(self.position))
        logger.log('')
    ...
    def _get_pos_by_name(self, location_name):
        ...
        if ',' in location_name:
            ...
            if len(possibleCoordinates) == 2:
                ...
                logger.log(
                    '[x] Ditemukan koordinat, bukan geocoding.')
                            ...
                            if 'inventory_item_data' in item:
                                if 'player_stats' in item[
                                    ...
                                    if 'pokemons_captured' in playerdata:
                                        logger.log(
                                            '[#] -- Nyekel Pokemon: {pokemons_captured}'.format(
                                                **playerdata))

                                    if 'poke_stop_visits' in playerdata:
                                        logger.log(
                                            '[#] -- Datang ke Pokestops : {poke_stop_visits}'.format(
                                                **playerdata))
