...
class PokemonCatchWorker(object):
            ...
            if 'ENCOUNTER' in response_dict['responses']:
                if 'status' in response_dict['responses']['ENCOUNTER']:
                    if response_dict['responses']['ENCOUNTER']['status'] is 7:
                        logger.log('[x] Pokemon Bag penuh!', 'red')
                        ...
                        while(True):
                            ...
                            if balls_stock[2] > 0:
                                if pokeball is 0 and cp <= 300 and balls_stock[2] < 10:
                                    logger.log('[-] Great Ball stock sedikit... simpan untuk cp diatas 300', 'yellow')
                            ...
                            if balls_stock[3] > 0:
                                if pokeball is 0 and cp <= 700 and balls_stock[3] < 10:
                                    logger.log('[-] Ultra Ball stock sedikit... simpan untuk cp diatas 700', 'yellow')
                            ...
                            if pokeball is 0:
                                logger.log(
                                    '[x] Pokeball habis, beralih ke mode farm..', 'red')
                            ...
                            logger.log('[x] gunakan {}... (sisa {}!)'.format(
                            ...
                            if response_dict and \
                                ...
                                if status is 2:
                                    logger.log(
                                        '[-] gagal mendapatkan {} - .. coba lagi!'.format(pokemon_name), 'red')
                                    sleep(2)
                                    continue
                                if status is 3:
                                    logger.log(
                                        '[x]  {} KABOOOOR!!!! :('.format(pokemon_name), 'red')
                                if status is 1:
                                    logger.log(
                                        '[x] Berhasil Mendapatkan {}! [CP {}] [IV {}] - Periksa Release Config'.format(
                                            pokemon_name,
                                            cp,
                                            pokemon_potential
                                        ), 'green'
                                    ...
                                    if self.should_release_pokemon(pokemon_name, cp, pokemon_potential, response_dict):
                                        ...
                                        logger.log(
                                            '[#] {} Ditukarkan candy!'.format(pokemon_name), 'green')
                                    else:
                                        logger.log(
                                        '[x] Mendapatkan {}! [CP {}]'.format(pokemon_name, cp), 'green')
                            break
        time.sleep(5)
    ...
    def should_release_pokemon(self, pokemon_name, cp, iv, response_dict):
        if self._check_always_capture_exception_for(pokemon_name):
            ...
            logger.log(
                "[x] Release config untuk {}: CP {} {} IV {}".format(
                    pokemon_name,
                    min_cp,
                    cp_iv_logic,
                    min_iv
